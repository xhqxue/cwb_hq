...
team_com <- function(x_vec, u_vec, workers_team, team_id) {
  # x <- NULL
  # u <- NULL
  com_times<-c(1:5)
  ...
  # return(list(x = x,u = u))
  #
  for(i in 1:length(workers_team)){
    com_id <- which(team_id == i)
    num_com_times <- sample(com_times,1)
    for(ii in 1:num_com_times){
      k <- sample(com_id, 2)
      xu_vec1 <- uncertain_effect(x_vec[k[1]], u_vec[k[1]], x_vec[k[2]], u_vec[k[2]])
      xu_vec2 <- uncertain_effect(x_vec[k[2]], u_vec[k[2]], x_vec[k[1]], u_vec[k[1]])
      x_vec[k[1]] <- xu_vec1[1]
