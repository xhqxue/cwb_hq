...
leader_com <- function(worker_vec, workers_team, i) {
  workers_team <- workers_team - 1
  n_team <- c(1:length(workers_team))
  num_com <- c(length(n_team)*0.4:length(n_team)*0.7)
  ...
  for(i in 1:n_com){
    ...
    t1_xu <- uncertain_effect(mate_th[t1], mate_th_u[t1], mate_th[t2], mate_th_u[t2])
    t2_xu <- uncertain_effect(mate_th[t2], mate_th_u[t2], mate_th[t1], mate_th_u[t1])
